...
for sub in $sublist; do
   ...
   if [ $kk -ge 1 ] && [ $kk -le 100 ] ; then
	...
	if [ ! -e $subdir/$sub/T1w/target_list_surf360.2mm.txt ]; then
	   	  
	   for i in $targetlist3; do
		echo $i >> $subdir/$sub/T1w/target_list_surf360.2mm.txt 
	   done
	fi
	...
	for roi in $roilist; do
		...
		probtrackx2_gpu8.0 -x $roidir/${roi}.asc  -V 1 -l --onewaycondition --loopcheck  -P 5000  \
		...
		--dir=$subdir/$sub/T1w/$resultdir/$roi  \
	        --targetmasks=$subdir/$sub/T1w/target_list_surf590.2mm.txt --os2t --s2tastext --ompl

		#--target2=$subdir/$sub/MNINonLinear/ROI_surf2/left.asc   --omatrix2 --omatrix1
				
                # --lrtarget3=$subdir/$sub/T1w/target_list_surf210.2mm.txt --target3=$roidir/${roi}.asc --omatrix3 \

		#--stop=$subdir/$sub/T1w/avoid_ventricle.2mm.txt   target_list_surf210.2mm.txt  surf2.2mm.txt 

...
[ result_reshape_dti_distance ] = reshape_cope_compact( result );

load result_dti-distance_task210_5k.mat
[ result_reshape_dti_distance_5k ] = reshape_cope_compact( result );


load result_dti_task210length_correction-xl.mat
[ result_reshape_dti_xl ] = reshape_cope_compact( result );

load result_dti_task210val5k.mat
[ result_reshape_dti5k ] = reshape_cope_compact( result );

load result_dti_task210length_correction5k.mat
[ result_reshape_dti_lc ] = reshape_cope_compact( result );
% 
load result_distance_task210_5k.mat
[ result_reshape_distance ] = reshape_cope_compact( result );

load result_distance_task210_xl.mat
[ result_reshape_distance_xl ] = reshape_cope_compact( result );
...
[ result_reshape_resting_distance ] = reshape_cope_compact( result );

load result_resting-distance_task210_5k.mat
[ result_reshape_resting_distance_5k ] = reshape_cope_compact( result );


% load result_resting210_sample20.mat
% [ result_reshape_resting ] = reshape_cope_compact( result );
% 
% load result_resting_4merge_task_volume.mat
% [ result_reshape_resting_volume ] = reshape_cope( result );
% 
% load result_dti_task_volume.mat
% [ result_reshape_dti_volume ] = reshape_cope( result );
...
%     x=result_reshape_dti{i}{m}(1,:);
%     y=result_reshape_dti5k{i}{m}(1,:);
    
%     x=result_reshape_distance{i}{m}(1,:);
%     y=result_reshape_distance_xl{i}{m}(1,:);

%     x=result_reshape_dti5k{i}{m}(1,:);
%     y=result_reshape_distance_xl{i}{m}(1,:);
    
%     x=result_reshape_dti5k{i}{m}(1,:);
%     y=result_reshape_distance{i}{m}(1,:);
 
%     x=result_reshape_dti5k{i}{m}(1,:);
%     y=result_reshape_dti_xl{i}{m}(1,:);
    
%     x=result_reshape_dti5k{i}{m}(1,:);
%     y=result_reshape_dti_distance{i}{m}(1,:);
    
%     x=result_reshape_dti5k{i}{m}(1,:);
%     y=result_reshape_dti_distance_5k{i}{m}(1,:);

        x=result_reshape_resting{i}{m}(1,:);
        y=result_reshape_distance{i}{m}(1,:);

%         x=result_reshape_resting{i}{m}(1,:);
%         y=result_reshape_resting_distance_5k{i}{m}(1,:);

%     x=result_reshape_resting_distance{i}{m}(1,:);
%     y=result_reshape_dti_distance{i}{m}(1,:);
...
%     xlabel(['Surface(FC) mean=',num2str(mean(x),2)],'FontName','Times New Roman','FontSize',15,'FontWeight','bold')
%     ylabel(['Volume(FC) mean=',num2str(mean(y),2)],'FontName','Times New Roman','FontSize',15,'FontWeight','bold')
%     xlabel('AC ','FontName','Times New Roman','FontSize',15,'FontWeight','bold')
%     ylabel('AC correction ','FontName','Times New Roman','FontSize',15,'FontWeight','bold')
    xlabel(['FC mean=',num2str(mean(x),2)],'FontName','Times New Roman','FontSize',15,'FontWeight','bold')
    ylabel(['Distance mean=',num2str(mean(y),2)],'FontName','Times New Roman','FontSize',15,'FontWeight','bold')
...
%     text(0.2,0.6,['r = ',num2str(corr(x',y'),2)],'FontName','Times New Roman','FontSize',15,'FontWeight','bold');
    ...
    [h_pair{i}{m},p_pair{i}{m},ci_pair{i}{m},stats_pair{i}{m}] = ttest(x,y,'Alpha',1e-4,'Tail','both');
    text(0.2,0.7,['h = ',num2str(h_pair{i}{m})],'FontName','Times New Roman','FontSize',15,'FontWeight','bold');
